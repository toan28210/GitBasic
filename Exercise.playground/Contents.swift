...
func changeCh(_ str: String) -> String? {
    ...
    let first = Character(firstCharacter)
    let last = Character(lastCharacter)
    
    newString.removeFirst()
    newString.removeLast()
    //insert
    newString.insert(last, at: (newString.startIndex))
    newString.insert(first, at: (newString.endIndex))
    return newString
}
...
multipleOf3Or5(number: 0)
//lesson 10
let str = "Helloadsadsa"
func addTwoCharacters(str: String) -> String{
    let lenght = str.count
    if lenght < 2 {
        return "The length of the string must be more than 2 characters"
    }
